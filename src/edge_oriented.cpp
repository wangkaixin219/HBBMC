...
EBBMC_Graph_t::EBBMC_Graph_t() = default;

EBBMC_Graph_t::~EBBMC_Graph_t() {
    int i;
...
void EBBMC_Graph_t::read_edges_from_file(const char *file_name) {
    FILE *fp;
...
void EBBMC_Graph_t::read_ordered_edges_from_file(const char *file_name) {
    FILE *fp;
    if ((fp = fopen(file_name, "r")) == nullptr) {
        printf("Cannot open file %s.\n", file_name);
        exit(0);
...
void EBBMC_t::truss_decompose(const char* w_file_name) {
...
void EBBMC_Graph_t::build_from_G() {
    int i;
...
void EBBMC_Graph_t::EBBMC(int l, unsigned long long *cliques) {
...
void EBBMC_Graph_t::EBBMC_plus(int l, unsigned long long *cliques) {
...
void EBBMC_Graph_t::EBBMC_plus_plus(int l, unsigned long long *cliques) {
...
void EBBMC_Comb_list(int *list, int list_size, int start, int picked, int k, unsigned long long *cliques) {
    if (picked == k) {
        (*cliques)++;
        return;
    }
    ...
    for (int i = start; i < list_size; i++) {
        EBBMC_Comb_list(list, list_size, i + 1, picked + 1, k, cliques);
    }
}

void EBBMC_Graph_t::list_in_plex(int start, int p, int q, unsigned long long *cliques) {
    if (F_size < q) return;

    if (p == 0) {
        if (q > F_size - q)
            EBBMC_Comb_list(F, F_size, 0, 0, F_size - q, cliques);
        else
            EBBMC_Comb_list(F, F_size, 0, 0, q, cliques);
        return;
    }
...
bool EBBMC_Graph_t::can_terminate(int l, unsigned long long *cliques) {
    ...
    if (sub_e_size[l] == sub_v_size[l] * (sub_v_size[l] - 1) / 2) {
        if (l > sub_v_size[l] - l)
            EBBMC_Comb_list(sub_v[l], sub_v_size[l], 0, 0, sub_v_size[l] - l, cliques);
        else
            EBBMC_Comb_list(sub_v[l], sub_v_size[l], 0, 0, l, cliques);
...
double EBBMC_t::truss_order(const char *r_file_name, const char *w_file_name) {
    double runtime;
...
double EBBMC_t::list_k_clique(const char *file_name) {
    double runtime;
    struct rusage start, end;
    EBBMC_Graph_t G;
